...
diag_log format ["*** rspawns minus the old fob %1", _respawns2];
_droppoint = [_respawns2, _oldv] call BIS_fnc_nearestPosition; //find the one nearest to the old forward
_fobid = _respawns find fobveh;
 if (_fobid > -1) then {[west, _fobid] call bis_fnc_removeRespawnPosition;};
...
sleep 1;
_nul = [_droppoint, "RHS_C130J", fobvehicle ] execVM "server\spawnairdrop.sqf";
