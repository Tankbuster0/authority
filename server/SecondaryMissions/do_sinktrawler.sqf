...
_smdir2 = [(_smnrtown getDir _missionpos)] call tky_fnc_cardinaldirection;
_smdist2 = [(_smnrtown distance2D _missionpos), 500] call tky_fnc_estimateddistance;
smmissionstring = format ["Native fishermen reported the presence of two unusual surface vessels %1 %2 of %3. Overhead imagery has determined them to be enemy and intelligence gathering or maybe even mine layers. Sink them.", _smdist2, _smdir2,_smnrtown ];
...
_ship1eh =  smship1 addEventHandler ["handledamage", "if ((_this select 3) in playableUnits) then {[_this select 3] call tky_fnc_shiphit}"];
_ship2eh =  smship2 addEventHandler ["handledamage", "if ((_this select 3) in playableUnits) then {[_this select 3] call tky_fnc_shiphit}"];
