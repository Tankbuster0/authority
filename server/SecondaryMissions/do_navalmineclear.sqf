...
__tky_starts;
private ["_smcleanup","_myplaces","_seapos1","_mfdata","_mfpos","_numberofmines","_nrquayobs","_myquayobj","_boatdir","_boatpos","_myboat","_minecounter","_chosenmine","_deepenough","_realminepos","_seadepth","_minespawnpos","_mine","_minemarkername","_helper","_m1","_mfreldir","_mfdist","_zz"];
...
_nrquayobs = (nearestTerrainObjects [cpt_position, ["house"], 2000, true, true]) select { ((getModelInfo _x) select 0) in boatspawnobjs };
if ((count (call tky_fnc_fleet_boats) < 1) and {(count _nrquayobs) > 0}) then
	...
	diag_log format ["*** nrqs count is %1, %2", count _nrquayobs, _nrquayobs];
	diag_log format ["*** myq is %1 at %2", _myquayobj, getpos _myquayobj];
	switch ((getModelInfo _myquayobj) select 0) do
		{
		case "pierconcrete_01_steps_f.p3d": {_boatdir = (getdir _myquayobj); _boatpos = [-7,-3,4];};
		case "pierconcrete_01_4m_ladders_f.p3d": {_boatdir = (getdir _myquayobj); _boatpos = [9,0,3];};
		case "pierwooden_02_ladder_f.p3d": {_boatdir = ((getdir _myquayobj) - 90), _boatpos = [0,4.5,18];};
		case "pierwooden_01_dock_f.p3d": {_boatdir = (getdir _myquayobj); _boatpos = [1.5,2,6];};
		case "pierwooden_01_hut_f.p3d": {_boatdir = ((getdir _myquayobj) + 90); _boatpos = [1,2,17];};
		case "pierwooden_03_f.p3d": {_boatdir = (getdir _myquayobj); _boatpos = [-1.6,6,19];};
		case "canal_dutch_01_stairs_f.p3d": {_boatdir = ((getdir _myquayobj ) + 90); _boatpos = [0,-11,4];};
		case "pier_small_f.p3d": {_boatdir = (getdir _myquayobj);  _boatpos =  [3.5,0,2];};
		case "canal_wall_stairs_f.p3d": {_boatdir = (getdir _myquayobj); _boatpos = [0,-4.2,0];};
		case "pier_addon_f.p3d": {_boatdir = selectRandom [((getdir _myquayobj) - 90), ((getdir _myquayobj) + 90)]; _boatpos = [0,3,-2];};
		};
	if ( (getTerrainHeightASL (_myquayobj modelToWorldWorld _boatpos)) < -1 ) then
		{
		_myboat = createvehicle ["B_Boat_Transport_01_F", [0,0,0], [],0,"NONE"];
		_myboat allowDamage false;
		_myboat setdir _boatdir;
		sleep 0.2;
		_myboat setPosWorld (_myquayobj modelToWorld _boatpos);
		sleep 0.2;
