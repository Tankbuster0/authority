...
flipactionid = player addAction ["Flip Vehicle", "client\unflipvehicle.sqf", "", 0, false, true, "", "cursorObject isKindOf 'LandVehicle' and {(((vectorup cursorObject) select 2) < 0.5) and (!(canmove cursorObject)) and ((player distance2D cursorObject) < 6)}" ];
__tky_ends