	...
	if ( GET_STATE( player ) == STATE_RESPAWNED ) then {
		...
		}else
		{
			h = [] spawn
			{
				...
				vehiclespawnerid = player addaction ["Make motorbike", "[_this select 0, _this select 1] remoteexec ['tky_fnc_spawnrunabout',2]","",0,false,false, "","((player distance blubasedataterminal) < 2)"];
			};
		};
