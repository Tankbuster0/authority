	...
	if ( GET_STATE( player ) == STATE_RESPAWNED ) then {
		...
		}else
		{
			h = [] spawn
			{
				...
				fobdeployactionid = player addaction ["Deploy FOB", "remoteexec ['tky_fnc_fobvehicledeploymanager',2]", "", 0,false,false, "", "(typeof (vehicle player) isEqualTo 'CUP_B_Mastiff_HMG_GB_W' ) and (round (speed vehicle player) isEqualTo 0) and (player isEqualTo (commander (vehicle player) ) )"];
			};
		};
