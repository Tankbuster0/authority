	...
	if ( GET_STATE( player ) == STATE_RESPAWNED ) then {
		...
		}else
		{
			h = [] spawn
			{
				...
				fobdeployactionid = player addaction ["Deploy/ Undeploy FOB", "remoteexec ['tky_fnc_fobvehicledeploymanager',2]", "", 0,false,false, "", "( (typeof (vehicle player) isEqualTo fobvehicleclassname )  and ((assignedVehicleRole player player) isEqualTo ['cargo'] ) and (not (isEngineOn (vehicle player))) ) "];
