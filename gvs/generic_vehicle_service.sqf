...
_per_fuel_time = if (count _this > 6) then { _this select 6 } else { 1 };
_per_repair_time = if (count _this > 7) then { _this select 7 } else { 1 };
_per_magazine_time = if (count _this > 8) then { _this select 8 } else { 1 };
...
if (_vName isEqualTo "") then {_vName = (_oVehicle getVariable ["d_ma_text", ""])};
_gtfo = 0;
{
...
if (_abort == 0) then
{
    ...
    Public_Banned_Vehicle_Service_List pushBack _vName;
